...
## Wifi Host Network Fallback
[Reference](https://www.raspberryconnect.com/projects/65-raspberrypi-hotspot-accesspoints/158-raspberry-pi-auto-wifi-hotspot-switch-direct-connection)
...
### Setup hostapd
```
sudo out=/etc/hostapd/hostapd.conf sh -c 'cat <<EOD > $out
...
### Setup dnsmasq
```
sudo out=/etc/dnsmasq.d/simtezilo.conf sh -c 'cat <<EOD > $out
#AutoHotspot Config
no-resolv
interface=wlan0
bind-interfaces
...
### Setup dhcpd
```
sudo out=/etc/dhcpd.conf sh -c 'cat <<EOD > $out
nohook wpa_supplicant
EOD'
```

### Setup autohotspot service
```
sudo out=/etc/systemd/system/autohotspot.service sh -c 'cat <<EOD > $out
[Unit]
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/opt/simtezilo/bin/autohotspot

[Install]
WantedBy=multi-user.target
EOD'

sudo mkdir -p /opt/simtezilo/bin

sudo out=/opt/simtezilo/bin/autohotspot sh -c 'cat <<EOD > $out
#!/bin/bash
#version 0.962-N/HS
...
sudo chmod 755 /opt/simtezilo/bin/autohotspot
